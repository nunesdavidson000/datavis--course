...
md`# D3 com Crossfilter e DC.js (Sincronização com Mapa)`
)});
...
facts.dimension( d => d.dtg)
  ...
  main.variable(observer("magDim")).define("magDim", ["facts"], function(facts){return(
facts.dimension( d => d.magnitude)
)});
  main.variable(observer("terremotoByMagnitude")).define("terremotoByMagnitude", ["magDim"], function(magDim){return(
magDim.group()
)});
...
md`${container()}`
  ...
  main.variable(observer("buildvis")).define("buildvis", ["dc","view","magDim","terremotoByMagnitude","d3","width","updateMarkers","depthDim","terremotoByDepth","hourDim","terremotoByHour","dataset","dateDim"], function(dc,view,magDim,terremotoByMagnitude,d3,width,updateMarkers,depthDim,terremotoByDepth,hourDim,terremotoByHour,dataset,dateDim)
{
  ...
  let bardepthChart = dc.barChart(view.querySelector("#depth-chart"))
  bardepthChart.width(480)
          .height(150)
          .dimension(depthDim)
...
idDimension.group()
  ...
  main.variable(observer("updateFilters")).define("updateFilters", ["idGrouping","layerList","map","idDimension","dc"], function(idGrouping,layerList,map,idDimension,dc){return(
function updateFilters(){
        var ids = idGrouping.all();
        var visibleMarkers = new Array(ids.length);
        var mc = 0
        layerList.eachLayer(function(layer){
