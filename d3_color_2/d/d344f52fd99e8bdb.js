...
md`# Taxa de desemprego nos EUA em agosto de 2016`
  ...
  main.variable(observer("buildvis")).define("buildvis", ["d3","DOM","green","topojson","us","rateById"], function(d3,DOM,green,topojson,us,rateById)
{
  const width = 960
  const height = 600
  ...
  svg.append("g")
    ...
    .enter().append("path")
      .attr("fill", d => scaleQuantize(rateById.get(d.id)))
      .attr("d", path)
      .append("title")
