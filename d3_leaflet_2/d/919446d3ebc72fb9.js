...
md`# D3 com Crossfilter e DC.js (Chicago)`
)});
...
d3.csv("https://gist.githubusercontent.com/emanueles/13dfe2c1d43e11b5207bb4e6125d71bf/raw/41d14f8fbd9d6cd9e8ad074f8417cff1329ab339/chicago_crimes_sept_2019.csv").then(function(data){
  let parseDate = d3.utcParse('%m/%d/%Y %H:%M:%S')
  
  data.forEach(function(d,i){
      d.dtg = parseDate(d.Date.substr(0,19))
      //+ porque eh valor numerio
      d.latitude = +d.Latitude
      d.longitude = +d.Longitude
      // [] para poder pegar a string Primary type
...
facts.dimension(d => d.dtg)
  ...
  main.variable(observer("tipoDim")).define("tipoDim", ["facts"], function(facts){return(
facts.dimension(d => d.type)
)});
  main.variable(observer("tipoGroup")).define("tipoGroup", ["tipoDim"], function(tipoDim){return(
tipoDim.group()
)});
  main.variable(observer("diaDim")).define("diaDim", ["facts","d3"], function(facts,d3){return(
facts.dimension(function(d){return [d.type,d3.timeDay(d.dtg)]})
)});
  main.variable(observer("diaGroup")).define("diaGroup", ["diaDim"], function(diaDim){return(
diaDim.group().reduceCount()
)});
  main.variable(observer("latitudeDim")).define("latitudeDim", ["facts"], function(facts){return(
facts.dimension(d => d.latitude)
)});
  main.variable(observer("longitudeDim")).define("longitudeDim", ["facts"], function(facts){return(
facts.dimension(d => d.longitude)
)});
...
d3.scaleOrdinal()
  ...
  main.variable(observer("buildvis")).define("buildvis", ["md","container","dc","tipoDim","d3","colorScale","tipoGroup","dataset","diaDim","diaGroup"], function(md,container,dc,tipoDim,d3,colorScale,tipoGroup,dataset,diaDim,diaGroup)
{
  let view = md`${container('dc-table-graph')}`
  
  let barChart = dc.barChart(view.querySelector("#bar-chart"))
  let seriesChart = dc.seriesChart(view.querySelector("#time-chart"))
  ...
  seriesChart.width(480)
           .height(150)
           .x(d3.scaleTime().domain(d3.extent(dataset, (d) => d3.timeDay(d.dtg))))
           .xUnits(d3.timeDays)
           .dimension(diaDim)
           .group(diaGroup)
