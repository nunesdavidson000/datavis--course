// https://observablehq.com/@nunesdavidson000/homicidios-em-fortaleza-em-2012@95
  ...
  main.variable(observer("geojson")).define("geojson", ["L","info","map","bairros","style"], function(L,info,map,bairros,style)
{
  ...
  main.variable(observer("legend")).define("legend", ["L","vermelho","colorScale","d3","map"], function(L,vermelho,colorScale,d3,map)
{
  let legendControl = L.control({position: 'bottomright'});
	...
	legendControl.onAdd = function (map) {
		...
		for (let i = 0; i < n; i++) {
			...
			labels.push(
				'<i style="background:' + vermelho[i] + '"></i> ' +
...
<main role="main" class="container">
 ...
 `
  ...
  main.variable(observer("vermelho")).define("vermelho", ["d3"], function(d3){return(
d3.schemeReds[7]
)});
  main.variable(observer("colorScale")).define("colorScale", ["d3","vermelho"], function(d3,vermelho){return(
d3.scaleQuantize()
    .domain([0, 70])
    .range(vermelho)
)});
...
d3.csv("https://gist.githubusercontent.com/emanueles/8ee225792454a78151d0842a10642a29/raw/9984e32effa99da3cea2ec87a7c9b7b231c71d3c/fortaleza_crimes_populacao.csv").then(function(data) {
    ...
    data.forEach(function(d) {
      if (d.Populacao != -1)
        bairrosMap.set(d.Bairro,+(d.Homicidios * d.Populacao/100000))
